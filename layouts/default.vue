<template>
  <div>
    <div class="header">HEADER</div>
    <div class="home-page">
      <div class="newsletter">
        <section>
          <h1>KEEP UPDATED</h1>
          <p>
            Sign up to our newsletter so you never miss out on the product you
            are looking for!
          </p>
        </section>
        <form method="POST">
          <input type="email" placeholder="Your email address" name="email" />
          <input type="submit" value="Sign Up" name="signNews" />
        </form>
      </div>
      <div class="container"></div>
      <footer>
        <div>
          <section>
            <div class="links">
              <a href="#"
                ><img
                  src="https://cdn-icons-png.flaticon.com/512/10794/10794710.png"
                  alt="logo-icon"
              /></a>
              <section class="href">
                <ul>
                  <li>About</li>
                  <li><a href="#">Our Story</a></li>
                  <li><a href="#">Careers</a></li>
                  <li><a href="#">Brand Guidelines</a></li>
                  <li><a href="#">Contact Us</a></li>
                </ul>
                <ul>
                  <li>Shop</li>
                  <li><a href="#">Help</a></li>
                  <li><a href="#">Downloads</a></li>
                  <li><a href="#">Shipping</a></li>
                  <li><a href="#">Return</a></li>
                </ul>
                <ul>
                  <li>Terms and Conditions</li>
                  <li><a href="#">All Terms and Policies</a></li>
                </ul>
              </section>
            </div>
            <div class="social">
              <section class="media">
                <a href="#"><i class="fa-brands fa-facebook-f"></i></a>
                <a href="#"><i class="fa-brands fa-twitter"></i></a>
                <a href="#"><i class="fa-brands fa-tiktok"></i></a>
                <a href="#"><i class="fa-brands fa-youtube"></i></a>
                <a href="#"><i class="fa-brands fa-instagram"></i></a>
                <a href="#"><i class="fa-brands fa-linkedin-in"></i></a>
                <a href="#"><i class="fa-brands fa-reddit-alien"></i></a>
                <a href="#"><i class="fa-brands fa-discord"></i></a>
                <a href="#"><i class="fa-brands fa-facebook-messenger"></i></a>
                <a href="#"><i class="fa-brands fa-whatsapp"></i></a>
              </section>
            </div>
          </section>
          <article>
            <p>Copyright @ 2023, All Right Reserved</p>
            <div class="lang">
              <i class="fa-solid fa-caret-down"></i>
              <p>English (EU)</p>
              <ul class="list-lang">
                <li>English (EU)</li>
                <li>Spanish (MX)</li>
                <li>Korean (KR)</li>
              </ul>
            </div>
          </article>
        </div>
      </footer>
    </div>
  </div>
</template>

<style>
@import url("https://fonts.googleapis.com/css2?family=Lato:ital,wght@0,100;0,300;0,400;0,700;0,900;1,100;1,300&display=swap");

@font-face {
  font-family: "Druk-Wide";
  src: url(../assets/fonts/Druk-Wide-Bold.ttf);
}

/* DEFAULT COLORS

$dark-blue;: #164B60;
$blue:#1B6B93;
$main-blue:#4FC0D0;
$pink:#FFD0D0;
$red:#DC0000;
$yellow:#FFF80A; */

* {
  padding: 0;
  margin: 0;
  font-family: "Lato", sans-serif;
}

html {
  scroll-behavior: smooth;
}

/* // footer  */

.home-page {
  display: flex;
  flex-direction: column;
  align-items: center;
  background-color: #4fc0d0;
  padding-top: 10vh;
}
footer {
  padding-top: 5vh;
  display: flex;
  justify-content: center;
  background: linear-gradient(180deg, #4fc0d0, #ffd0d0);
  width: 100%;
}

footer > div {
  width: 85%;
}

footer section {
  display: flex;
  justify-content: space-between;
  padding-bottom: 4vh;
}

footer article {
  display: flex;
  justify-content: space-between;
  border-top: 1px solid #1b6b93;
  color: lightgrey;
  padding: 5vh 0;
}

footer article > p {
  padding: 2vh 4vh;
  color: #164b60;
}

.links {
  display: flex;
  flex-direction: column;
  width: 50%;
  gap: 5vh;
}

.links img {
  width: 12vh;
  height: 12vh;
}

.href li {
  list-style: none;
  color: #164b60;
  padding: 0.4rem;
}

.href a {
  text-decoration: none;
  color: #1b6b93;
}

.href a:hover {
  color: #164b60;
  transition: 0.4s;
}

.href ul > li:first-child {
  font-weight: 700;
}

.href {
  display: flex;
  justify-content: space-between;
}

.media i:hover {
  background-color: #1b6b93;
  transition: 0.5s;
}

.media i {
  width: 8vh;
  height: 8vh;
  display: flex;
  justify-content: center;
  align-items: center;
  border-radius: 10px;
  color: #ffd0d0;
  background-color: #164b60;
}

.media {
  display: flex;
  flex-wrap: wrap;
  gap: 2vh;
  padding-top: 14vh;
}

.media > a {
  text-decoration: none;
}

.social {
  width: 28%;
}

.lang {
  display: flex;
  gap: 2vh;
  padding: 2vh 4vh;
  font-weight: 600;
  position: relative;
  color: #164b60;
}

.lang:hover {
  background-color: #164b60;
  cursor: pointer;
  color: #ffd0d0;
}

/* // Language selection - set js function */
.list-lang {
  display: none;
  position: absolute;
  flex-direction: column;
  background-color: #164b60;
  bottom: 100%;
  width: 100%;
  left: 0;
}

.list-lang > li {
  padding: 1vh;
  font-weight: 600;
  list-style: none;
}

.list-lang > li:hover {
  cursor: pointer;
  background-color: #4fc0d0;
}

.container {
  height: 100px;
}

.newsletter > form {
  padding: 2vh 5vh 2vh 0;
  display: flex;
  gap: 2vh;
}

.newsletter > section {
  padding: 2vh 0 2vh 5vh;
  width: 40%;
  display: flex;
  flex-direction: column;
  gap: 1vh;
}

.newsletter h1 {
  font-size: 2.3rem;
  background: linear-gradient(90deg, #ffd0d0, rgb(183, 75, 101));
  -webkit-background-clip: text;
  background-clip: text;
  -webkit-text-fill-color: transparent;
  font-family: "Druk-Wide", sans-serif;
}

.newsletter p {
  color: #ffd0d0;
  font-size: 1.1rem;
}

.newsletter input[type="submit"] {
  color: #1b1919;
  background-color: gainsboro;
  padding: 2vh 3vh;
  font-weight: 600;
  border: 1px solid gainsboro;
  border-radius: 8px;
  font-size: 1rem;
}

.newsletter input[type="submit"]:hover {
  background-color: #ffd0d0;
  cursor: pointer;
  transition: 0.4s;
}

.newsletter input[type="email"] {
  width: 50vh;
  padding: 1vh 2vh;
  background-color: rgb(53, 53, 70);
  border: 2px solid grey;
  border-radius: 8px;
  color: whitesmoke;
}

.newsletter ::placeholder {
  color: grey;
  font-weight: 600;
}

.newsletter {
  display: flex;
  width: 85%;
  align-items: center;
  background-color: #164b60;
  border: 2px solid #1b6b93;
  border-radius: 10px;
  justify-content: space-between;
  padding: 4vh 0;
}
</style>
